...
%Simulation of ZFa-inducible and ZFi-DsRed-inhibitable gene expression.
%    The ZFi-DsRed effect is represented as both competitive and affecting
%    cooperative RNAPII recruitment.


function sim = model_ZFa_ZFiDsRed_dual(doseZFa, doseZFiDsRed, m, wA, wI, b, l, u, Z)


%Input arguments:
%    doseZFa:      ZFa plasmid (ng)
%    doseZFiDsRed: ZFiDsRed plasmid (ng)
%    m:            maximum activation parameter
%    wA:           steepness parameter for ZFa
%    wI:           steepness parameter for ZFi-DsRed
...
%    sim = model_ZFa_ZFiDsRed_dual(50, 50, 32.7, 0.036, 0.036, 0.08, 0.5, 2, Z(c, :));

%Example for a homogeneous simulation:
%    sim = model_ZFa_ZFiDsRed_dual(50, 50, 32.7, 0.036, 0.036, 0.08, 0.5, 2, [1, 1, 1]);
...
%intercellular variation for the ZFi-DsRed
zid = Z(2);
...
[~, sim] = ode15s(@(t, y, options)[ 
    ...
    %Y3 ZFi-DsRed RNA
      ktxCMV   * zid * doseZFiDsRed                                    ... %constitutive transcription
    - kdegR    * y(3)                                                      %RNA degradation
    
    %Y4 ZFi-DsRed protein
      ktl      * y(3)                                                  ... %translation
    - kdegZFP  * y(4)                                                      %protein degradation    
    
    %Y5 Reporter RNA
      ktxZF    * zrep * (b + max(min(((4*wI * y(4)) ./ (wA * y(2)) - 4*l) ...
          * (1 - m) / (4*u - 4*l) + m, m), 1) * wA .* y(2))            ...
          ./ (1 + wA * y(2) + 4*wI * y(4))                             ... %inducible and inhibitable transcription
